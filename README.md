# Hospital Readmission Prediction - Diabetes 130-US Hospitals

[![Dataset](https://img.shields.io/badge/Dataset-UCI%20ML%20Repository%20%23296-blue?logo=data:image/svg+xml;base64,)](https://archive.ics.uci.edu/dataset/296/diabetes+130-us+hospitals+for+years+1999-2008)
[![CI](https://github.com/juliettebm/hospital-readmission-diabetes/actions/workflows/ci.yml/badge.svg)](https://github.com/juliettebm/hospital-readmission-diabetes/actions/workflows/ci.yml)
[![Python](https://img.shields.io/badge/Python-3.x-blue?logo=python&logoColor=white)](https://www.python.org/)
[![scikit-learn](https://img.shields.io/badge/scikit--learn-Random%20Forest-orange?logo=scikit-learn&logoColor=white)](https://scikit-learn.org/)
[![Streamlit](https://img.shields.io/badge/Streamlit-App-red?logo=streamlit&logoColor=white)](https://streamlit.io/)

Predictive modelling project for hospital readmission risk (any delay after discharge) in diabetic patients, built on the [Diabetes 130-US Hospitals (1999-2008)](https://archive.ics.uci.edu/dataset/296/diabetes+130-us+hospitals+for+years+1999-2008) dataset (UCI Machine Learning Repository).

---

## Objective

Predict whether a diabetic patient will be readmitted to hospital after an encounter, using only three clinically interpretable composite dimensions derived from administrative data.

> **Target definition.** The target is *any* readmission (`<30` or `>30` days), not the standard 30-day outcome: 46% of encounters are positive, versus about 11% for `<30` alone. Results are therefore **not comparable** with published 30-day readmission benchmarks.

---

## Project Structure

```
hospital-readmission-diabetes/
│
├── data/
│   ├── diabetic_data.csv              # Raw dataset (UCI), not tracked by Git
│   ├── diabetic_data_clean.csv        # After cleaning (output of notebook 01)
│   └── features.csv                   # Engineered features (output of notebook 02)
│
├── models/
│   ├── readmission_model.pkl          # Trained Random Forest classifier
│   ├── feature_params.pkl             # MinMaxScaler parameters (train set only)
│   └── metrics.json                    # Machine-readable reference metrics
│
├── Plot/                              # All saved visualisations
│
├── notebooks/
│   ├── 01_data_preparation.ipynb      # Data loading, cleaning, univariate analysis
│   ├── 02_feature_engineering.ipynb   # Feature construction, correlation analysis
│   └── 03_modelling.ipynb             # Model training, evaluation, explainability
│
├── app.py                             # Educational Streamlit demonstrator (not clinically validated)
├── src/readmission/                    # Tested feature and artifact-loading code
├── tests/                              # Unit, artifact smoke and consistency tests
├── .github/workflows/ci.yml           # Continuous integration
│
├── .gitignore
├── pyproject.toml                     # Pytest configuration
├── requirements.txt
├── requirements-dev.txt
└── README.md
```

---

## Reproduce

### 1. Clone the repository

```bash
git clone https://github.com/juliettebm/hospital-readmission-diabetes.git
cd hospital-readmission-diabetes
```

### 2. Install dependencies

```bash
pip install -r requirements.txt
```

### 3. Download the dataset

The raw dataset is not tracked by Git (101,766 rows, ~23 MB). Download it manually from the [UCI Machine Learning Repository](https://archive.ics.uci.edu/dataset/296/diabetes+130-us+hospitals+for+years+1999-2008) and place `diabetic_data.csv` in the `data/` folder.

### 4. Run the notebooks in order

The notebooks use paths relative to the `notebooks/` directory:

```bash
cd notebooks
jupyter notebook 01_data_preparation.ipynb
jupyter notebook 02_feature_engineering.ipynb
jupyter notebook 03_modelling.ipynb
cd ..
```

### 5. Launch the Streamlit app

```bash
streamlit run app.py
```

> The app loads `models/readmission_model.pkl` and `models/feature_params.pkl` (generated by notebooks 02 and 03, and versioned so the demo runs out of the box). It applies the same scaling and weighting as notebook 02 and stops with an explicit message if either file is missing; there is no heuristic fallback.

### 6. Run the automated checks

```bash
pip install -r requirements-dev.txt
pytest -q
```

The tests cover feature construction, out-of-range clipping, artifact loading and prediction, and consistency of the key metrics across `models/metrics.json`, the modelling notebook and this README.

---

## Notebooks

### `01_data_preparation.ipynb`

- Loads the raw dataset (101,766 encounters from 71,518 distinct patients, 50 variables)
- Detects and replaces disguised missing values (`'?'` → `NaN`)
- Drops columns with >60% missing data
- Performs univariate analysis on 8 key clinical variables
- Exports `diabetic_data_clean.csv`

### `02_feature_engineering.ipynb`

- Train/test split performed **before** any feature engineering and **by patient** (no leakage across encounters of the same patient)
- Correlation analysis on training set only (Pearson / point-biserial)
- Expert-driven dimensionality reduction: 7 variables → 3 clinical dimensions
- MinMaxScaling fitted on training set, applied to test set
- Correlation-based weighting within each dimension
- Uses the tested `src/readmission/features.py` implementation shared with the app
- Exports `features.csv` and `feature_params.pkl`

### `03_modelling.ipynb`

- Reconstructs train/test split from the `split` column in `features.csv`
- Trains a DummyClassifier naive baseline (`strategy='most_frequent'`)
- Trains a Random Forest with 5-fold patient-grouped cross-validation on the training set
- Evaluates on the held-out test set (ROC-AUC, sensitivity, specificity)
- Produces evaluation dashboard: confusion matrix, ROC curve, feature importance
- Exports `readmission_model.pkl`

---

## Clinical Framework

The 7 original numeric variables are reduced to 3 interpretable dimensions:

| Dimension                | Variables                                                   | Clinical Concept                        |
| ------------------------ | ----------------------------------------------------------- | --------------------------------------- |
| **Pathological Terrain** | `number_diagnoses`                                          | Comorbidity burden (cf. Charlson score) |
| **Chronic Instability**  | `number_inpatient`, `number_emergency`, `number_outpatient` | Past healthcare utilisation             |
| **Episode Severity**     | `time_in_hospital`, `num_lab_procedures`, `num_medications` | Current episode intensity               |

Each dimension is built by MinMax scaling on the training set, then correlation-weighted aggregation.

---

## Results

| Metric                      | Value                               |
| --------------------------- | ----------------------------------- |
| Naive baseline AUC          | 0.500                               |
| Random Forest AUC (grouped CV) | 0.648 ± 0.002                    |
| Random Forest AUC (test set)| **0.649**                           |
| Train set                   | 81,613 encounters (57,214 patients) |
| Test set                    | 20,153 encounters (14,304 patients) |
| Class balance               | 54% not readmitted / 46% readmitted |

The split and the cross-validation are **grouped by patient** (`patient_nbr`): the dataset contains several encounters per patient, and a row-level split would let the same patient appear in both train and test. (Switching from a row-level to a grouped split left the AUC unchanged at 0.649.)

**Chronic Instability** (prior healthcare utilisation) is the strongest predictor (~63% feature importance), consistent with established clinical evidence.

`models/metrics.json` records these reference results in machine-readable form. CI fails if the values documented here diverge from that file or from the saved output of the modelling notebook.

### Reproducibility status

| Check | Status |
| --- | --- |
| Exact Python dependency versions | Automated install in CI |
| Unit tests for feature construction | Automated in CI |
| Bundled pickle load and prediction | Automated smoke test in CI |
| README/notebook metric consistency | Automated in CI |
| Full training from raw UCI data | Manual; dataset downloaded separately |
| Calibration and external validation | Not yet performed |

---

## Methodological Choices

- **No PCA**: expert-driven reduction preserves clinical interpretability
- **Train/test split before feature engineering**: strict leakage prevention
- **`class_weight='balanced'`**: handles class imbalance without synthetic data generation
- **Correlation weighting** used as a pragmatic signal-strength heuristic, not a causal claim
- **DummyClassifier baseline**: defines a performance floor for clinical relevance assessment

## Limitations and next steps

- The output concerns **any later readmission**, not 30-day readmission.
- An AUC of 0.649 indicates modest discrimination; this educational demonstrator is not suitable for clinical decision-making.
- The displayed model score is not presented as a validated clinical probability. Calibration curves, Brier score and bootstrap confidence intervals should be generated when the modelling notebook is next rerun from source data.
- External validation and decision-curve analysis would be required before making claims about clinical utility.

---

## Stack

```
Python 3.x · pandas · numpy · scikit-learn · matplotlib · seaborn · joblib · streamlit
```

---

## License

Released under the [MIT License](LICENSE). The *Diabetes 130-US Hospitals* dataset is distributed by the UCI Machine Learning Repository under its own [terms of use](https://archive.ics.uci.edu/dataset/296/diabetes+130-us+hospitals+for+years+1999-2008); it is downloaded separately and not redistributed here.

---

## Data Source

Strack, B., DeShazo, J.P., Gennings, C., et al. (2014). *Impact of HbA1c Measurement on Hospital Readmission Rates: Analysis of 70,000 Clinical Database Patient Records.* BioMed Research International.  
Dataset: [UCI Machine Learning Repository #296](https://archive.ics.uci.edu/dataset/296/diabetes+130-us+hospitals+for+years+1999-2008)

---

## See also

[patient-pathway-analysis](https://github.com/juliettebm/patient-pathway-analysis): Healthcare analytics dashboard & statistical inference module (Python, SQL, Streamlit)

---

## Author

**Juliette Bouli-Mengue**
Clinical Research to Data Science
